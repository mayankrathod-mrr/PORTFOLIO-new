...
const experienceData = [
  {
    ...
    company: "Self-Employed / Freelance", // <-- ADDED THIS LINE
  ...
  return (
    <section id="experience" className="py-24">
      <div className="container mx-auto px-6">
        <h2 className="text-5xl font-bold text-center text-gray-800 dark:text-gray-100 mb-16">
          My Journey
        </h2>
        <div className="relative max-w-4xl mx-auto">
          {/* The vertical line */}
          <div className="absolute left-1/2 w-0.5 h-full bg-blue-200 dark:bg-blue-800 transform -translate-x-1/2"></div>
            ...
            {experienceData.map((item, index) => (
              <div key={index} className="flex justify-between items-center w-full">
                ...
                <div className={`w-1/2 ${index % 2 === 0 ? 'pr-8' : ''}`}>
                  {index % 2 === 0 && (
                    <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg text-right">
                      <p className="text-sm text-blue-500 dark:text-blue-400 font-semibold">{item.date}</p>
                      <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">{item.role}</h3>
                      {/* We only show the company if it exists */}
                      {item.company && <p className="text-md text-gray-600 dark:text-gray-300 mb-2">{item.company}</p>}
                ...
                <div className="absolute left-1/2 -translate-x-1/2 w-8 h-8 bg-blue-500 rounded-full border-4 border-white dark:border-gray-900 z-10"></div>

                {/* Right Side Content (or Spacer) */}
                <div className={`w-1/2 ${index % 2 !== 0 ? 'pl-8' : ''}`}>
                  {index % 2 !== 0 && (
                    <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg text-left">
                      <p className="text-sm text-blue-500 dark:text-blue-400 font-semibold">{item.date}</p>
                      <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">{item.role}</h3>
                      {/* We only show the company if it exists */}
                      {item.company && <p className="text-md text-gray-600 dark:text-gray-300 mb-2">{item.company}</p>}
